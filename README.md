...
│   ├── router/       # 路由配置
│   └── v1/           # API v1 版本接口（文章、用户、评论、标签、分类、友链、动态、菜单、通知、反馈、订阅、RSS、AI、系统等）
├── cmd/              # 应用入口
│   └── main.go
├── config/           # 配置管理
├── docs/             # Swagger 生成的文档
...
│   ├── model/        # 数据模型
│   ├── repository/   # 数据访问层
│   └── service/      # 业务逻辑层（文章、用户、评论、标签、分类、友链、动态、菜单、通知、反馈、订阅、RSS、系统、配置等）
├── pkg/              # 可复用的包
│   ├── database/     # 数据库连接管理
│   ├── email/        # 邮件发送
│   ├── errcode/      # 统一错误码
│   ├── feishu/       # 飞书 SDK
│   ├── notification/ # 通知服务（邮件/飞书聚合）
│   ├── response/     # 统一响应格式
│   ├── scheduler/    # 定时任务调度
│   └── upload/       # 文件上传管理
├── templates/        # 模板文件
...
- **移动端目录优化**
  - 减小抽屉宽度至75%（原80%），最大280px（原320px）
  - 优化标题和内边距，减少空间占用
  - 标题字号从18px调整为16px
  - 头部内边距从20px调整为16px
  - 内容区内边距从10px调整为8px
  - 目录项内边距从10px 20px调整为10px 16px
  - 各级标题缩进优化，更紧凑合理
  - 添加小屏幕（≤360px）专属优化
  - 添加 overscroll-behavior: contain 防止滚动穿透
...
- **代码块复制功能优化**
  ...
  - 兼容所有协议环境（HTTPS、HTTP、localhost、IP地址访问）
  - 确保长代码块复制时内容完整且不重复

- **大文章导出超时优化**
  - 将导出超时时间从5分钟增加到10分钟
  - 优化等待提示：5秒后显示打包提示，30秒后显示进度提示
  - 改进用户体验，避免长时间无反馈
  - 适应大文章和多图片资源的导出场景
